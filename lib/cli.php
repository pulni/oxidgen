...
abstract class CLI {
  ...
  public function printSeperator( $seperator = "" ) {
    if ( $seperator ) {
      print_r( str_repeat( $seperator, 80 ) );
    }
    echo "\n";
  }
   ...
   * @return  void
   */
  public function printError( $message, $newLine = true ) {
    $this->printMessage( $this->colorText($message, self::LIGHT_RED), $newLine );
  }
    ...
    foreach ( $this->args as $type => $values ) {
      foreach ( $values as $k => $v ) {
        ...
        if ( method_exists( $this, $method ) ) {
          // check if we can handle over arguments
          $args = is_numeric( $k ) ? true : $v;
